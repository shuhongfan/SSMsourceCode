...
public class DefaultListableBeanFactory extends AbstractAutowireCapableBeanFactory
	...
	//bean定义的一个Map，key是bean的名称，值是 bean的定义
	...
	/**
	 * 创建一个默认的DefaultListableBeanFactory，使用给定的父上下文（父bean factory）
	 *
	...
	public void preInstantiateSingletons() throws BeansException {
		...
		// bean初始化后回调，对bean的进一步处理
		// Trigger post-initialization callback for all applicable beans...
		for (String beanName : beanNames) {

			Object singletonInstance = getSingleton(beanName);

			//当所有单例bean都初始化完成以后，容器会回调该接口的方法 afterSingletonsInstantiated，可以在该回调中做一些事情
	...
	public void registerBeanDefinition(String beanName, BeanDefinition beanDefinition)
		...
		//从beanDefinitionMap中获取一下是否存在该bean的定义
		...
		if (existingDefinition != null) {
			...
			//TODO 把beanDefinition放入beanDefinitionMap的一个Map中，key是beanName，值是beanDefinition对象
			...
			if (hasBeanCreationStarted()) {
				...
				synchronized (this.beanDefinitionMap) {
					...
					//把bean的定义beanDefinition放入map中
					this.beanDefinitionMap.put(beanName, beanDefinition);

					//把bean的定义的名字放入beanDefinitionNames的List中
					List<String> updatedDefinitions = new ArrayList<>(this.beanDefinitionNames.size() + 1);
					updatedDefinitions.addAll(this.beanDefinitionNames);
					updatedDefinitions.add(beanName);
					this.beanDefinitionNames = updatedDefinitions;

					//删除手动的bean集合中的bean的名字
					removeManualSingletonName(beanName);
				}
			}
			else {
				//TODO 开始注册bean，beanDefinitionMap的Map中注册bean，key为bean的id，value为BeanDefinition
	...
	public Object doResolveDependency(DependencyDescriptor descriptor, @Nullable String beanName,
		...
		try {
			...
			if (instanceCandidate instanceof Class) {
				//TODO 解析候选者bean，会调beanFactory.getBean()方法获取bean，该方法内部会进行初始化
