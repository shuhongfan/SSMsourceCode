...
public class UrlPathHelper {
	...
	public String getPathWithinServletMapping(HttpServletRequest request) {
		String pathWithinApp = getPathWithinApplication(request);//获得controller方法上的路径
		String servletPath = getServletPath(request);//获得服务path路径
		String sanitizedPathWithinApp = getSanitizedPath(pathWithinApp);//获取去掉两个斜杠的//的路径，替换成一个斜杠
		String path;

		// If the app container sanitized the servletPath, check against the sanitized version
		if (servletPath.contains(sanitizedPathWithinApp)) {
			path = getRemainingPath(sanitizedPathWithinApp, servletPath, false);//得到剩下的url
		}
		else {
	...
	public String getPathWithinApplication(HttpServletRequest request) {
		String contextPath = getContextPath(request);//获取项目上下文
		String requestUri = getRequestUri(request);//获取请求uri
		String path = getRemainingPath(requestUri, contextPath, true);//获得controller方法上的请求路径
		if (path != null) {
	...
	public String getServletPath(HttpServletRequest request) {
		...
		if (servletPath == null) {
			servletPath = request.getServletPath();//获取服务path路径
		}
	...
	private String decodeAndCleanUriString(HttpServletRequest request, String uri) {
		...
		uri = getSanitizedPath(uri);//路径消毒，//双斜杠替换成/单斜杠
		return uri;
	}
