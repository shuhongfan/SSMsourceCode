...
public class DispatcherServlet extends FrameworkServlet {
	...
	protected void initStrategies(ApplicationContext context) {
		...
		initLocaleResolver(context);//初始化支持国际化区域解析器
		initThemeResolver(context);//初始化更换主题的支持
		initHandlerMappings(context);//初始化handler映射
		initHandlerAdapters(context);//初始化handler适配器
		initHandlerExceptionResolvers(context);//异常处理解析器
		initRequestToViewNameTranslator(context);//用于直接将请求转换为逻辑视图名。
	...
	/**
	 * 初始化HandlerMappings，如果在BeanFactory中没有HandlerMappings定义，那么默认会使用BeanNameUrlHandlerMapping
	 *
	...
	private void initHandlerMappings(ApplicationContext context) {
		...
		if (this.detectAllHandlerMappings) {
			...
			if (!matchingBeans.isEmpty()) {
				...
				// We keep HandlerMappings in sorted order. 排序一下
	...
	protected void doDispatch(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			try {
				...
				// TODO 确定当前请求的HandlerExecutionChain执行链（也就是controller的执行方法）
				...
				// TODO 确定当前请求的handler适配器  RequestMappingHandlerAdapter
	...
	protected View resolveViewName(String viewName, @Nullable Map<String, Object> model,
		...
		if (this.viewResolvers != null) {
			for (ViewResolver viewResolver : this.viewResolvers) {
				View view = viewResolver.resolveViewName(viewName, locale);//创建视图对象View
				if (view != null) {
					return view;
				}
			}
